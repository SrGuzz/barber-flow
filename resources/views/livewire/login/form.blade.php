<div class="relative justify-content bg-gray-50 p-4 rounded-lg shadow-lg ">
       
    <x-form wire:submit="login" >
 
        {{-- SIGN IN GOOGLE --}}
        <a href="{{ route('auth.google') }}"
            class="btn btn-outline w-full mt-8 flex items-center justify-center">
            Login com Google
            <span class="h-4 w-4 block">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
                    <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
                    <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
                    <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
                    <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
                    <path fill="none" d="M0 0h48v48H0z"/>
                </svg>
            </span>
        </a>
        
        {{-- LINE --}}
        <div class="divider text-sm"> ou </div>
        
        <x-input
        label="E-mail de usuário"
        type="email"
        wire:model="mail"/>
        
        <x-password
        label="Senha"
        wire:model="password"
        password-icon="o-lock-closed"
        password-visible-icon="o-lock-open" />    
        
        <div class="flex mt-1 justify-between text-sm">
            <x-checkbox label="Lembrar-me" wire:model="remember" />
            <a href="/forgot-password" class="hover:underline">
                Esqueceu a senha?   
            </a>
        </div>
            
        <div wire:ignore class="p-6">
            <script src="https://www.google.com/recaptcha/api.js" async defer></script>
            <div 
            class="g-recaptcha" 
            data-sitekey="{{ env('RECAPTCHA_SITE_KEY') }}" 
            data-callback="recaptchaCallback"
            style="  display: flex; justify-content: center;">
            </div>  
        </div>
    
        <input type="hidden" id="recaptcha" wire:model="recaptcha">
        
        @error('recaptcha')
        <p class="text-red-500 text-sm italic text-center">{{ $message }}</p>
        @enderror
        
        <script>
            function recaptchaCallback(token) {
                const recaptchaInput = document.getElementById('recaptcha');
                recaptchaInput.value = token;
                
                // event input
                recaptchaInput.dispatchEvent(new Event('input', { bubbles: true }));
            } 
            </script>
            
            <x-button 
            type="submit" 
            class="btn-primary w-full mb-6" 
            spinner="login">
            Entrar
        </x-button> 
        
    </x-form>
    
    <div class="flex items-center justify-center text-sm"> 
        <a href="/sign-up" class="hover:underline">
            Não tem uma conta? Cadastre-se
        </a>
    </div>
    
    
</div>

